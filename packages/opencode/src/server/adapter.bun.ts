...
function listen(app: FetchApp, opts: Opts, websocket?: ReturnType<typeof createBunWebSocket>["websocket"]) {
  const tls = opts.tls ? { tls: { cert: opts.tls.cert, key: opts.tls.key } } : {}
  const start = (port: number) => {
    try {
      if (websocket) {
        return Bun.serve({ fetch: app.fetch, hostname: opts.hostname, idleTimeout: 0, websocket, port, ...tls })
      }
      return Bun.serve({ fetch: app.fetch, hostname: opts.hostname, idleTimeout: 0, port, ...tls })
    } catch {
      return
    }
  }

...
export const RunCommand = effectCmd({
  ...
  // --attach connects to a remote server (no local instance needed). The local
  // path manages its own instance via bootstrap() so it can prepare a Container
  // runtime first and bind the (possibly relocated) directory to the instance.
  instance: false,
  builder: (yargs: Argv) =>
    yargs
      .positional("message", {

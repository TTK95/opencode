...
- Upstream CI runs on Blacksmith runners the fork doesn't have: `test`, `typecheck`, and `nix-eval` fall back to GitHub-hosted runners (`ubuntu-24.04` / `windows-2025`) via a `runs-on` expression when not in `anomalyco/opencode`; `generate` (needs the upstream app secret, pushes bot commits to dev) and the final `publish` job in `publish.yml` (its `always() && !failure()` condition passed even with all dependencies skipped) are repo-gated.

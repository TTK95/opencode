...
## 10. Auto-sync upstream + fork self-update channel
...
- Upstream CI runs on Blacksmith runners the fork doesn't have: `typecheck` and `nix-eval` fall back to GitHub-hosted `ubuntu-24.04` via a `runs-on` expression; `test` runs unit (linux) only on forks via a repository-conditional matrix — the windows matrix and the `e2e` job are upstream-only (windows tests assume the Blacksmith environment; Playwright's browser install hangs on GitHub-hosted runners under both bunx and npx); `generate` (needs the upstream app secret, pushes bot commits to dev) and the final `publish` job in `publish.yml` (its `always() && !failure()` condition passed even with all dependencies skipped) are repo-gated.
